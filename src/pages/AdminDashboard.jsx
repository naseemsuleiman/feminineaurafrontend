...
import {
  ...
  PiggyBank, FileText, Quote, Calendar, Wallet,
  BarChart3, ArrowUpRight, Heart,
} from 'lucide-react';
import API from '../api';
...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      series, totalSaved, articlesByCat,
    };
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen grid place-items-center text-mulberry/60">
        <div className="text-center">
          <Sparkles className="animate-pulse text-rose-400 mx-auto mb-3" size={28} />
          <p className="font-serif text-lg">Loading your sanctuary…</p>
        </div>
      </div>
    );
  }
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-10 md:py-14">
        ...
        <div className="flex flex-wrap items-end justify-between gap-6 mb-10">
          <div>
            <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/70 backdrop-blur border border-blush-200 text-rose-600 text-[10px] uppercase tracking-[0.3em] font-medium mb-4">
              <Crown size={11} /> Admin Console
            </span>
            <h1 className="font-serif text-4xl md:text-5xl leading-[1.05] text-mulberry">
              Welcome back,
              <br />
        ...
        {tab === 'overview' && (
          <div className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
              ...
              <div className="lg:col-span-2 relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-rose-600 via-rose-400 to-blush-200 p-8 text-white shadow-card">
                <div className="absolute -top-16 -right-16 w-56 h-56 rounded-full bg-white/25 blur-3xl" />
                <div className="relative">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-2 text-white/85 text-[10px] uppercase tracking-[0.3em]">
                      <Users size={13} /> Total Subscribers
                    </div>
                    <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/20 text-[10px] uppercase tracking-widest">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-300 animate-pulse" />
                      Live
                    </span>
                  </div>
                  <p className="font-serif text-7xl leading-none">{subs.count}</p>
            ...
            {/* Row 2: Chart + Site Status */}
            <div className="grid lg:grid-cols-3 gap-5">
              {/* Weekly chart */}
              <div className="lg:col-span-2 card">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-mulberry/50">
                    <TrendingUp size={12} className="text-rose-600" /> Last 7 Days
                  </div>
                  <span className="text-xs text-mulberry/50">
                    {stats.weekCount} new
                  </span>
                </div>
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-2">
                        <div className="w-full h-full flex items-end">
                          <div
                            className="w-full rounded-t-xl bg-gradient-to-t from-rose-600 to-rose-400 transition-all duration-500"
                            style={{ height: `${Math.max(height, 4)}%` }}
                            title={`${d.count} on ${d.label}`}
                          />
                        </div>
                        <p className="text-[10px] uppercase tracking-widest text-mulberry/40">
                          {d.label}
                        </p>
                        <p className="text-xs font-medium text-mulberry">{d.count}</p>
                      </div>
                    );
                  })}
                </div>
              ...
              {/* Site status */}
              <div className="card">
                <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-mulberry/50 mb-5">
                ...
                {siteLive === null ? (
                  ...
                  <>
                    <div className="flex items-center gap-3 mb-3">
                      <span className="w-3 h-3 rounded-full bg-green-500 shadow-[0_0_0_6px_rgba(74,222,128,0.2)]" />
                      <p className="font-serif text-3xl text-mulberry">Live</p>
                    </div>
                  ...
                  <>
                    <div className="flex items-center gap-3 mb-3">
                      <span className="w-3 h-3 rounded-full bg-rose-500 shadow-[0_0_0_6px_rgba(200,122,139,0.2)] animate-pulse" />
                      <p className="font-serif text-3xl text-mulberry">Coming Soon</p>
                    </div>
                    <p className="text-mulberry/60 text-sm">
                      Toggle in{' '}
                      <a
                        href="https://feminineaurabackend.onrender.com/admin/"
                        target="_blank"
                        rel="noreferrer"
            ...
            {/* Row 3: Recent subscribers + Quick export */}
            <div className="grid lg:grid-cols-3 gap-5">
              <div className="lg:col-span-2 card">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-mulberry/50">
                    <Users size={12} className="text-rose-600" /> Recent Subscribers
                  </div>
                  <button
                    ...
                    {subs.subscribers.slice(0, 5).map((s) => (
                      <li key={s.id} className="flex items-center justify-between py-3">
                        <div className="flex items-center gap-3">
                          <span className="w-8 h-8 rounded-full bg-gradient-to-br from-rose-400 to-rose-600 grid place-items-center text-white text-xs font-medium">
                            {s.email.charAt(0).toUpperCase()}
                          </span>
                          <span className="text-sm text-mulberry font-medium">
                            {s.email}
                          </span>
                        </div>
                        <span className="text-xs text-mulberry/50">
              ...
              <div className="card flex flex-col">
                ...
                <p className="font-serif text-3xl text-mulberry">
                ...
                <button
                  onClick={exportCSV}
                  disabled={!subs.count}
                  className="btn-primary mt-6 w-full !py-2.5 text-sm disabled:opacity-40"
                >
                  Export CSV
                </button>
              </div>
        ...
        {tab === 'subscribers' && (
          <div className="card overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div>
                <h2 className="font-serif text-2xl">All Subscribers</h2>
        ...
        {tab === 'budgets' && (
          <div className="space-y-5">
            ...
            <div className="card">
              <h2 className="font-serif text-2xl mb-6">Budgets Overview</h2>
              {budgets.length === 0 ? (
        ...
        {tab === 'content' && (
          <div className="space-y-5">
            ...
            <div className="card">
              <h2 className="font-serif text-2xl mb-6">Recent Articles</h2>
              {articles.length === 0 ? (
                ...
                <ul className="divide-y divide-blush-100">
                  {articles.slice(0, 10).map((a) => (
                    <li key={a.id} className="py-4 flex items-start gap-4">
                      <span className="text-[10px] uppercase tracking-widest text-rose-600 pt-1 w-20 shrink-0">
                        {a.category}
                      </span>
                      <div className="flex-1">
                        <p className="font-serif text-lg text-mulberry">{a.title}</p>
                        <p className="text-sm text-mulberry/60 line-clamp-2 mt-1">
                          {a.excerpt || a.body?.slice(0, 120)}
                        </p>
                      </div>
                      <span className="text-xs text-mulberry/40 shrink-0">
...
function StatCard({ icon: Icon, label, value, sub }) {
  return (
    <div className="card relative overflow-hidden">
      ...
      <div className="relative">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-mulberry/50 font-medium mb-5">
          <Icon size={12} className="text-rose-600" /> {label}
        </div>
        <p className="font-serif text-5xl text-mulberry">{value}</p>
        <p className="text-mulberry/60 text-sm mt-3">{sub}</p>
      </div>
    </div>
  );
}
...
function GlanceCard({ icon: Icon, title, value, sub }) {
  return (
    <div className="card">
      <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-mulberry/50 font-medium mb-5">
        <Icon size={12} className="text-rose-600" /> {title}
      </div>
      <p className="font-serif text-3xl text-mulberry">{value}</p>
      <p className="text-mulberry/60 text-sm mt-2">{sub}</p>
    </div>
  );
}
...
function EmptyState({ icon: Icon, title }) {
  return (
    <div className="text-center py-12">
      <Icon size={28} className="mx-auto text-rose-400 mb-3" />
      <p className="font-serif text-lg text-mulberry">{title}</p>
    </div>
  );
}