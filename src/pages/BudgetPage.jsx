...
export default function BudgetPage() {
  ...
  useEffect(() => {
    if (paymentStatus === 'success') {
      setNotice({
        type: 'success',
        text: 'Payment received! Unlocking your tracker…',
      });
      // Refresh user so has_paid flips to true
      loadUser();
      // Give the webhook a moment, then reload user again
    ...
    if (paymentStatus === 'cancelled') {
      setNotice({
        type: 'info',
        text: 'Checkout cancelled. You can try again anytime.',
      });
    }
...
const handleUnlock = async () => {
    setPaying(true);
    try {
      // This calls your Django backend, which talks to Paystack
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-16">
        ...
        {/* Month picker (only when user has access) */}
        {user && hasAccess && (
          ...
          <>
            {isAdmin && (
              <div className="max-w-2xl mx-auto mb-6 flex items-center gap-2 px-4 py-3 rounded-2xl bg-white/70 border border-blush-200 text-xs uppercase tracking-widest text-rose-600">
                <Crown size={13} /> Admin preview — you have full access
              </div>
            )}
        ...
        {/* ─────────────── USER, NO ACCESS → PREVIEW + PAYWALL ─────────────── */}
        {user && !hasAccess && (
          <BudgetPreview
            month={month}
            price={PRICE_LABEL}
...
function BudgetPreview({ month, price, onUnlock, paying }) {
  return (
    <div className="relative max-w-5xl mx-auto">
      ...
      <div className="select-none pointer-events-none blur-[6px] opacity-60 transition">
        <div className="rounded-3xl bg-white p-6 md:p-8 shadow-card border border-blush-100">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div>
              <h3 className="font-serif text-2xl text-mulberry">
                30-Day Tracker
              </h3>
          ...
          <div className="space-y-2.5">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="grid grid-cols-6 gap-3 items-center text-sm"
              >
                <div className="font-medium text-mulberry/70 text-xs">
                  Day {i + 1}
                </div>
          ...
          <div className="mt-8 grid grid-cols-3 gap-4">
            <div className="rounded-2xl bg-blush-50 p-4">
              <p className="text-[10px] uppercase tracking-widest text-mulberry/50">
                Savings
              </p>
      ...
      <div className="absolute inset-0 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center bg-white/95 backdrop-blur rounded-[2rem] p-8 md:p-10 shadow-card border border-blush-100">
          ...
          <p className="text-mulberry/65 leading-relaxed mb-6">
            ...
            wants, and savings with grace and intention. One payment, yours
            forever.
          </p>
          ...
          <ul className="text-left space-y-2.5 mb-7 max-w-xs mx-auto">
            {[
              ...
              <li
                key={f}
                className="flex items-start gap-2.5 text-sm text-mulberry/75"
              >
                <Check
                  size={15}
